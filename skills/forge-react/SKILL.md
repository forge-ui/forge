...
4. **Layout uses `<AppLayout>`. For page columns, spans and responsive spacing, read [layout-grid](references/layout-grid.md) and use `Grid` / `GridItem`; keep one-dimensional toolbars in Flex.** Don't assemble sidebar + topbar from scratch. Auth forms currently have no ready-made Forge template; compose them from `TextField`, `Checkbox`, `Button`, and `StyledLink`.
