...
- `references/component-registry-lite.json`：核心 ForgeUI 组件语义注册。用它判断什么时候用 `DataTable`、`SurfaceCard`、`StatCard`、`ButtonGroup`、`Label`、`ProgressBar`、`FileCard`、`HistoryItem`、form controls。
...
- Dashboard 的重点数字可用 `text-3xl` 或 Forge stat 组件默认尺度；普通卡片标题用 `text-lg` / `text-base`，表格主文本用 `text-sm`，辅助元信息和 filter label 用 `text-xs`。
