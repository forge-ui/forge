...
```text
请用 Forge starter 创建一个新的后台原型项目，starter 地址是 https://github.com/forge-ui/forge-starter。
请使用 Forge Skill 和 forge-app-design 插件来完成这件事：Forge Skill 负责约束组件库、token、AppLayout 和基础样式；forge-app-design 负责把需求梳理成页面意图、信息架构、业务模块和验收清单。
先 clone starter 并初始化环境，确认 @forge-ui-official/core、Tailwind v4、pnpm dev/typecheck/build 都可用。
环境就绪后，先主动向我确认业务目标、用户角色、需要的页面/模块、核心数据对象、主要操作流程和验收标准。
我确认后，再开始设计并实现业务模块。
实现时优先使用 AppLayout 和 ForgeUI 组件，不要手写基础 Button/Card/Table/Form/Layout 样式；页面布局可以按业务需要设计，但基础颜色、字体、圆角、边框、密度和响应式行为必须遵循 ForgeUI。
