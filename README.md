...
**太长不看：把下面这段直接丢给 Codex。**

```text
请用 Forge starter 创建一个新的后台原型项目，starter 地址是 https://github.com/forge-ui/forge-starter。
先 clone 并初始化环境，确认 @forge-ui-official/core、Tailwind v4 和基础运行脚本可用。
环境就绪后，先主动向我确认业务目标、用户角色、需要的页面/模块、核心数据对象和主要操作流程。
我确认后，再开始设计并实现业务模块。
实现时优先使用 AppLayout 和 ForgeUI 组件，不要手写基础 Button/Card/Table/Form/Layout 样式。
完成后运行 pnpm typecheck 和 pnpm build，并给出可访问地址和页面截图。
```
